...
import { isPublicKey } from "./transaction.js";
/**
...
function readAmount(data, offset, encoding) {
...
function matchRule(policy, programId, dataHex) {
    let best, programAllowed = false;
    for (const rule of policy.programs) {
        if (rule.programId !== programId)
            continue;
        programAllowed = true;
        if (!dataHex.startsWith(rule.discriminator))
            continue;
 ...
 */
export function evaluatePolicy(decoded, policy, expectedFeePayer) {
    ...
    const spend = new Map();
    let explicitLimit, unitPrice;
    for (const ix of decoded.instructions) {
        const { rule, programAllowed } = matchRule(policy, ix.programId, ix.dataHex);
        if (!programAllowed)
            throw Error("policy_program");
        if (!rule)
            throw Error("policy_instruction");
        if (rule.effect === "none")
            continue;
        if (rule.effect === "fee") {
        ...
        const { asset, amountOffset, amountEncoding, mintAccountIndex } = rule.spend;
        if (mintAccountIndex !== undefined) {
            const account = ix.accounts[mintAccountIndex];
            if (account === undefined)
                throw Error("policy_spend_mint_missing");
    ...
    const computeUnitLimit = explicitLimit ??
        Math.min(DEFAULT_UNITS_PER_INSTRUCTION * decoded.instructions.length, MAX_UNITS_PER_TRANSACTION);
    ...
    const priorityFeeLamports = (BigInt(computeUnitLimit) * microLamports + 999999n) / 1000000n;
    ...
    if (policy.maxPriorityFeeBps !== undefined) {
        ...
        if (nativeLeg !== undefined && nativeLeg > 0n) {
            const ceiling = (nativeLeg * BigInt(policy.maxPriorityFeeBps)) / 10000n;
            if (priorityFeeLamports > ceiling)
                throw Error("policy_priority_fee");
        }
    }
    for (const [asset, total] of spend) {
        ...
        if (typeof cap !== "bigint")
            throw Error("policy_cap_missing");
        if (total > cap)
            throw Error("policy_spend_cap");
    }
    return {
        spend,
        computeUnitLimit,
...
function parseCount(value, label, max) {
...
function parseProgramRule(x) {
    if (!x || typeof x !== "object")
        throw Error("policy_program_invalid");
    const r = x;
    if (!isPublicKey(r.programId))
        throw Error("policy_program_invalid");
    if (typeof r.discriminator !== "string" ||
    ...
    if (r.effect === "fee") {
        ...
        if (r.discriminator !== SET_UNIT_LIMIT &&
            r.discriminator !== SET_UNIT_PRICE)
            throw Error("policy_fee_effect_invalid");
        return { ...base, effect: "fee" };
    }
    if (r.effect !== "spend")
        throw Error("policy_effect_invalid");
    const s = r.spend;
 ...
 */
export async function loadSignPolicy(path) {
    const st = await lstat(path);
    if (st.isSymbolicLink())
        throw Error("policy_symlink_forbidden");
    if (!st.isFile())
        throw Error("policy_format_invalid");
    if (permissionsAreUnsafe(st))
        throw Error("policy_permissions_unsafe");
    ...
    if (x.version !== 1 ||
        ...
        throw Error("policy_format_invalid");
    const programs = x.programs.map(parseProgramRule), seen = new Set();
    for (const p of programs) {
        const key = `${p.programId}:${p.discriminator}`;
        if (seen.has(key))
            throw Error("policy_program_duplicate");
        seen.add(key);
    }
    const caps = {};
    for (const [asset, value] of Object.entries(x.caps)) {
    ...
    return {
        ...
        maxRequiredSignatures: parseCount(x.maxRequiredSignatures, "max_required_signatures", 8),
        maxComputeUnitLimit: parseCount(x.maxComputeUnitLimit, "max_compute_unit_limit", MAX_UNITS_PER_TRANSACTION),
        maxComputeUnitPriceMicroLamports: parseUnsigned(x.maxComputeUnitPriceMicroLamports, "max_compute_unit_price"),
        maxPriorityFeeLamports: parseUnsigned(x.maxPriorityFeeLamports, "max_priority_fee"),
        ...(x.maxPriorityFeeBps === undefined
            ? {}
            : {
                maxPriorityFeeBps: parseCount(x.maxPriorityFeeBps, "max_priority_fee_bps", 10_000),
            }),
        addressLookupTables: [...x.addressLookupTables],
