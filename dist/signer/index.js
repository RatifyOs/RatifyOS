import { SignerWireVerifier, } from "./authorization.js";
import { evaluatePolicy } from "./policy.js";
import { attachSignature, decodeTransaction, signatureOf, } from "./transaction.js";
...
export class SignerService {
    ...
    policy;
    wire;
    constructor(account, replay, policy, wire) {
        this.account = account;
        this.replay = replay;
        this.policy = policy;
        this.wire = wire;
    }
    stored(id, requestHash) {
     ...
     */
    async fenceBlockhash(id, lastValidBlockHeight) {
        if (lastValidBlockHeight === undefined || !this.wire?.blockHeight)
            return;
        ...
        if (height <= lastValidBlockHeight)
            return;
        await this.replay.transition?.(id, "claimed", "expired", JSON.stringify({ lastValidBlockHeight, blockHeight: height }));
     ...
     */
    async sign(authorizationId, transaction, claimedFeePayer, lastValidBlockHeight) {
        const decoded = decodeTransaction(transaction);
        evaluatePolicy(decoded, this.policy, this.account.publicKey);
        if (decoded.feePayer !== claimedFeePayer)
    ...
    async signClaimed(id, decoded, lastValidBlockHeight) {
        try {
            const raw = this.account.signMessage(decoded.messageBytes), result = attachSignature(decoded, raw), data = JSON.stringify({
     ...
     * exactly one caller.
     */
    result(id, requestHash, recoverRaw = false) {
        if (this.replay.get?.(id)?.state === "expired")
            return { state: "expired" };
        const data = this.stored(id, requestHash);
        if (!data)
            return { state: "not_found" };
        const result = { state: "signed", signature: data.signature };
        if (recoverRaw) {
            const claimed = this.replay.recoverSigned?.(id);
            if (claimed) {
                let recovered;
                try {
     ...
     * authorization id.
     */
    async signEnvelope(transaction, envelope) {
        if (!this.wire)
            throw Error("wire_verifier_required");
        const decoded = decodeTransaction(transaction);
 ...
 */
export async function broadcastSigned(store, id, transaction, rpc) {
    const expected = signatureOf(transaction);
    let lastValidBlockHeight;
    try {
        const prior = JSON.parse(store.get(id)?.data ?? "{}");
    ...
    if (typeof returned !== "string" || returned !== expected) {
        await store.transition(id, "signed", "reconciliation", JSON.stringify({ expected, returned, lastValidBlockHeight }));
        throw Error("broadcast_signature_mismatch");
    }
    if (!(await store.transition(id, "signed", "broadcast", JSON.stringify({
        signature: returned,
        transaction,
        lastValidBlockHeight,
    }))))
 ...
 */
export async function reconcileTransactions(store, rpc) {
    for (const row of store.list(["broadcast", "reconciliation"])) {
            ...
            const settled = status.err
                ...
                : status.confirmationStatus === "finalized" ||
                    ...
                    : row.state === "reconciliation"
                        ...
                        : undefined;
            if (settled)
                await store.transition(row.id, row.state, settled, JSON.stringify({
                    signature,
                    status,
        ...
        if (!Number.isSafeInteger(data.lastValidBlockHeight))
            continue;
        ...
        if (Number.isSafeInteger(height) && height > data.lastValidBlockHeight)
            await store.transition(row.id, row.state, "dropped", JSON.stringify({
                signature,
                lastValidBlockHeight: data.lastValidBlockHeight,
                blockHeight: height,
            }));
