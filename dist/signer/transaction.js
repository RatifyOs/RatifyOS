 ...
 */
export function decodeTransaction(wireBase64) {
    ...
    if (!Buffer.from(canonical).equals(raw))
        throw Error("transaction_encoding_invalid");
    const message = tx.message, staticKeys = message.staticAccountKeys.map((k) => k.toBase58()), header = message.header;
    if (!staticKeys.length)
        throw Error("transaction_accounts_invalid");
    if (!Number.isSafeInteger(header.numRequiredSignatures) ||
    ...
    if (!compiled.length)
        throw Error("transaction_instructions_empty");
    ...
    const lookedUp = lookups.reduce((n, l) => n + l.writableIndexes.length + l.readonlyIndexes.length, 0);
    const totalKeys = staticKeys.length + lookedUp;
    const instructions = compiled.map((ix) => {
        if (!Number.isSafeInteger(ix.programIdIndex) ||
            ix.programIdIndex < 0 ||
    ...
    return {
        ...
        signatures: tx.signatures.map((s) => EMPTY_SIGNATURE(s) ? null : bs58.encode(s)),
 ...
 */
export function attachSignature(decoded, signature) {
    if (signature.length !== 64)
        throw Error("signature_invalid");
    const tx = new VersionedTransaction(decoded.transaction.message, decoded.transaction.signatures.map((s) => Uint8Array.from(s)));
...
export function signatureOf(wireBase64) {
    const first = decodeTransaction(wireBase64).signatures[0];
    if (!first)
        throw Error("signature_missing");
    return first;
}
