...
const json = (x) => JSON.stringify(x, (_k, v) => typeof v === "bigint" ? { $bigint: v.toString() } : v);
const parse = (x) => JSON.parse(x, (_k, v) => v && typeof v === "object" && "$bigint" in v ? BigInt(v.$bigint) : v);
const digest = (x) => createHash("sha256").update(json(x)).digest("hex");
export class ExecutionStore {
    db;
    constructor(path) {
        this.db = new DatabaseSync(path);
        this.db.exec(`PRAGMA journal_mode=WAL; PRAGMA synchronous=FULL; PRAGMA busy_timeout=5000; CREATE TABLE IF NOT EXISTS executions(id TEXT PRIMARY KEY,idempotency_key TEXT UNIQUE NOT NULL,payload TEXT NOT NULL,state TEXT);CREATE TABLE IF NOT EXISTS quotes(id TEXT PRIMARY KEY,payload TEXT NOT NULL,quote_hash TEXT)`);
        ...
        if (!executionColumns.some((c) => c.name === "state")) {
            ...
            for (const row of this.db
                ...
                this.db
                    ...
                    .run(parse(String(row.payload)).state, row.id);
        }
        const quoteColumns = this.db.prepare("PRAGMA table_info(quotes)").all();
        if (!quoteColumns.some((c) => c.name === "quote_hash")) {
            ...
            for (const row of this.db
                ...
                this.db
                    ...
                    .run(parse(String(row.payload)).quoteHash, row.id);
        }
        this.db.exec("CREATE INDEX IF NOT EXISTS executions_state ON executions(state); CREATE INDEX IF NOT EXISTS quotes_quote_hash ON quotes(quote_hash)");
    }
    putQuote(q) {
        this.db
            .prepare("INSERT OR REPLACE INTO quotes(id,payload,quote_hash) VALUES(?,?,?)")
    ...
    getQuote(id) {
        const r = this.db
            ...
            .get(id);
        return r ? parse(String(r.payload)) : undefined;
    }
    findQuoteByHash(hash) {
        const r = this.db
            .prepare("SELECT payload FROM quotes WHERE quote_hash=? ORDER BY rowid LIMIT 1")
            .get(hash);
        return r ? parse(String(r.payload)) : undefined;
    }
    create(x, state = "awaiting-approval") {
        ...
        this.db
            .prepare("INSERT INTO executions(id,idempotency_key,payload,state) VALUES(?,?,?,?)")
    ...
    list(states) {
        ...
        return this.db
            .prepare(`SELECT payload FROM executions WHERE state IN (${marks}) ORDER BY rowid`)
            .all(...states).map((r) => parse(String(r.payload)));
    }
    transition(id, from, p, version) {
        this.db.exec("BEGIN IMMEDIATE");
        try {
            const old = this.get(id);
            if (!old)
                throw Error("execution_not_found");
            if (!(Array.isArray(from) ? from : [from]).includes(old.state) ||
            ...
            const r = this.db
                .prepare("UPDATE executions SET payload=?,state=? WHERE id=? AND payload=?")
...
export class TradingOrchestrator {
    ...
    async quote(raw) {
        ...
        const inputMint = this.mint(raw.inputMint, "inputMint"), outputMint = this.mint(raw.outputMint, "outputMint"), allowed = this.c.policy.allowedMints;
        if (allowed &&
            (!allowed.includes(inputMint) || !allowed.includes(outputMint)))
            throw Error("mint denied");
        const r = await this.c.rpc.quote({ ...raw, inputMint, outputMint }), policyHash = this.policyHash, minimumOut = (r.amountOut * BigInt(10000 - raw.slippageBps)) / 10000n;
        const core = {
            side: raw.side,
     ...
     */
    async revokeQuote(tokenAccountRaw) {
        const tokenAccount = this.mint(tokenAccountRaw, "tokenAccount");
        if (!this.c.rpc.revokeQuote)
            throw Error("revoke_unsupported");
        const r = await this.c.rpc.revokeQuote({
            tokenAccount,
            owner: this.c.account,
        });
        if (!r.request || !r.evidence)
            throw Error("exact_transaction_required");
    ...
    async execute(quoteId, o) {
        ...
        if (q.request) {
            ...
            if (!("success" in got) || !got.success)
                throw Error("simulation_failed");
        }
        if (dryRun)
        ...
        const approvalId = x.id, b = this.binding(q, evidence);
    ...
    assertExact(id, r) {
        const x = this.require(id), q = this.c.store.getQuote(x.quoteId);
        if (!q?.request ||
    ...
    approve(id, operator, input) {
        const x = this.require(id);
        if (x.actor === operator)
            throw Error("self approval prohibited");
        if (x.state !== "awaiting-approval")
            throw Error("not awaiting approval");
        if (input && x.approvalId)
            this.c.approvalEngine?.decide(x.approvalId, {
                ...input,
                operatorId: operator,
            });
        else if (this.c.approvalEngine)
            throw Error("decision proof required");
        return this.refreshApproval(id, operator);
    }
    deny(id, operator, input) {
        const x = this.require(id);
        if (x.state !== "awaiting-approval")
            throw Error("not awaiting approval");
    ...
    refreshApproval(id, operator) {
        ...
        if (!a)
            throw Error("approval record missing");
        if (a.status === "denied")
    ...
    async submit(id) {
        ...
        if (x.state !== "approved" && x.state !== "signing")
            throw Error(x.state === "submitting" ? "submission in progress" : "not approved");
        const q = this.c.store.getQuote(x.quoteId);
        if (!q?.request || !q.evidence)
            throw Error("exact_transaction_required");
        ...
        try {
            const env = await this.c.authorizationIssuer.issue(q.request, q.evidence, {
                quoteHash: q.quoteHash,
                policyHash: q.policyHash,
            ...
            const landed = signed.broadcast ?? (await this.c.rpc.broadcast(signed.transaction));
            if (landed !== signature)
     ...
     */
    async recoverSigning(row) {
        const q = this.c.store.getQuote(row.quoteId);
        if (!q?.request)
            throw Error("exact_transaction_required");
        if (!row.authorizationId)
            return this.submit(row.id);
        if (!this.c.signer?.result)
            throw Error("signer_result_unavailable");
        ...
        if (signed.state === "not_found")
            throw Error("signer_result_not_found");
    ...
    async recoverAndReconcile(limit = 100) {
        const rows = this.c.store
            ...
            .slice(0, Math.max(0, limit)), result = { scanned: rows.length, recovered: 0, reconciled: 0, failed: 0 };
    ...
    async reconcile(id) {
        const x = this.require(id);
        if (!x.signature)
            throw Error("transaction_not_broadcast");
        const status = await this.c.rpc.status(x.signature);
        if (!status) {
            if (x.lastValidBlockHeight !== undefined &&
...
function assertSignedTransaction(signed) {
    if (typeof signed?.transaction !== "string" ||
 ...
 */
export class UnixSignerClient {
    socketPath;
    authorizationToken;
    timeoutMs;
    constructor(socketPath, authorizationToken = "", timeoutMs = 5000) {
    ...
    request(payload) {
        return new Promise((resolve, reject) => {
            ...
            let data = "";
            const timer = setTimeout(() => s.destroy(Error("signer_timeout")), this.timeoutMs);
            s.on("connect", () => s.write(json({ token: this.authorizationToken, ...payload }) +
                "\n"));
            s.on("data", (b) => {
                data += b;
    ...
    async sign(request) {
        ...
        const signature = assertSignedTransaction(v);
