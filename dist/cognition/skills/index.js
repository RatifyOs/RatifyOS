...
const SOURCES = ["workspace", "local", "bundled"];
const NAME = /^[a-z0-9][a-z0-9_-]{0,63}$/;
const ENV = /^[A-Za-z_][A-Za-z0-9_]{0,127}$/;
const BIN = /^[A-Za-z0-9][A-Za-z0-9._+-]{0,127}$/;
const CONFIG = /^[A-Za-z][A-Za-z0-9_-]{0,63}(?:\.[A-Za-z][A-Za-z0-9_-]{0,63}){0,15}$/;
...
const MAX_MANIFEST = 256 * 1024, MAX_FILE = 1024 * 1024, MAX_FILES = 256, MAX_LIST = 64;
function parse(text) {
...
function configured(config, path) {
    let v = config;
...
async function binaryExists(name, pathValue) {
    if (!BIN.test(name) || pathValue === undefined)
        return false;
...
async function readNoFollow(path, base, max = MAX_FILE) {
...
const posixKey = (path) => path.split(/[\\/]/).join("/");
const posixKeyed = (files) => new Map(Object.entries(files).map(([k, v]) => [posixKey(k), v]));
async function supportFiles(dir, base) {
    const out = new Map();
    for (const top of SUPPORT) {
        ...
        for (const ent of entries) {
            ...
            const full = join(ent.parentPath, ent.name), rel = relative(dir, full);
            out.set(posixKey(rel), await readNoFollow(full, base));
        }
    }
    return out;
}
function digest(text, files) {
...
export class SkillManager {
    ...
    async selected() {
        ...
        for (const source of SOURCES) {
            ...
            for (const entry of entries) {
                ...
                const text = await readNoFollow(manifest, base, MAX_MANIFEST), parsed = parse(text);
    ...
    async metadata(item) {
        const p = item.parsed, missingEnv = p.requires.env.filter((n) => !this.env[n]), missingConfig = p.requires.config.filter((n) => !configured(this.config, n)), missingBinaries = [];
        for (const n of p.requires.binaries)
            if (!(await binaryExists(n, this.env.PATH)))
                missingBinaries.push(n);
        return frozen({
            name: p.name,
    ...
    async load(name, pin = {}) {
        ...
        return frozen({
            ...
            suspicious: suspicious(item.text + "\n" + [...item.support.values()].join("\n")),
        });
    }
    ...
    async controlled(name) {
        if (!this.controlledRoot)
            throw new Error("No controlled root configured");
        if (!NAME.test(name))
            throw new Error("Invalid skill name");
        const lexical = resolve(this.controlledRoot);
        const ls = await lstat(lexical);
        if (ls.isSymbolicLink() || !ls.isDirectory())
            throw new Error("Controlled root must be a real directory, not a symlink");
    ...
    async create(name, text, files = {}) {
        const target = await this.controlled(name), p = parse(text);
        if (p.name !== name)
    ...
    async update(name, text, options = {}) {
        const target = await this.controlled(name), base = await realpath(target);
        if ((await lstat(target)).isSymbolicLink() ||
            (enforceRealpathIdentity && base !== target))
            throw new Error("Symlink target rejected");
        const current = await readNoFollow(join(target, "SKILL.md"), base, MAX_MANIFEST), currentFiles = await supportFiles(target, base), currentDigest = digest(current, currentFiles);
        if (options.expectedChecksum && options.expectedChecksum !== currentDigest)
            throw new Error("Skill checksum pin mismatch");
        const p = parse(text);
        if (p.name !== name)
            throw new Error("Skill name mismatch");
        const id = randomUUID(), temp = `${target}.tmp-${id}`, backup = `${target}.bak-${id}`;
    ...
    async writeTree(target, text, files) {
        ...
        try {
            ...
            const values = [["SKILL.md", text], ...all];
            for (const [path, value] of values) {
                const file = join(target, path);
                await mkdir(dirname(file), { recursive: true });
                const fh = await open(file, constants.O_WRONLY |
                    constants.O_CREAT |
                    constants.O_EXCL |
...
async function syncDir(path) {
    ...
    try {
        ...
        const code = e.code ?? "";
        if (SYNC_DIR_UNSUPPORTED.has(code) || code === "EACCES")
            return;
        throw e;
    }
    try {
        ...
        if (!SYNC_DIR_UNSUPPORTED.has(e.code ?? ""))
            throw e;
    }
    finally {
        await dh.close();
