...
function capabilityFor(tool, surface) {
    if (surface === "preview")
        return TRADING_CAPABILITIES.ORDER_SIMULATE;
 ...
 */
export function registerIntentTool(registry, tool, runtime) {
    ...
    registry.register({
        ...
        execute: async (config) => {
            ...
            const outcome = await tool.execute(ctx, config, {
                ...
                confirmedByUser: (await runtime.confirmedByUser?.(tool.name, config)) === true,
            });
            if (outcome.isError)
...
export function registerIntentTools(registry, tools, runtime) {
    for (const tool of tools)
        registerIntentTool(registry, tool, runtime);
    return registry;
}
