...
function canonical(v, seen = new Set()) {
    ...
    if (Array.isArray(v)) {
        if (seen.has(v))
            throw new Error("Invalid JSON: cycle");
        seen.add(v);
...
INSERT INTO messages_fts(messages_fts) VALUES('rebuild');`);
                ...
                if (!/no such module/i.test(String(e)))
                    throw e;
                ...
                this.db.exec("DROP TRIGGER IF EXISTS messages_ai; DROP TRIGGER IF EXISTS messages_ad;");
            }
            this.db.exec("COMMIT");
        }
        catch (e) {
    ...
    journalMode() {
        return text(this.db.prepare("PRAGMA journal_mode").get(), "journal_mode").toLowerCase();
    }
    close() {
        this.db.close();
    }
    createSession(i) {
        ...
        this.db
            ...
            .run(i.id, i.title ?? null, i.parentSessionId ?? null, i.parentMessageId ?? null, at);
    ...
    appendMessage(i) {
        this.db
            .prepare("INSERT INTO messages(id,session_id,role,content,created_at)VALUES(?,?,?,?,?) ON CONFLICT DO NOTHING")
            .run(i.id, i.sessionId, i.role, i.content, i.createdAt);
        const r = this.db.prepare("SELECT * FROM messages WHERE id=?").get(i.id);
        if (!r ||
    ...
    getMessages(id, o = {}) {
        if (!o.includeAncestors)
            return this.db
                ...
                .all(id).map((r) => this.mm(r));
        const seen = new Set(), line = [];
        let s = this.getSession(id);
        while (s) {
            if (seen.has(s.id))
                throw new Error("Session lineage cycle");
            seen.add(s.id);
            line.unshift(s);
        ...
        const out = [];
        line.forEach((x, n) => {
            const rows = this.getMessages(x.id), cut = line[n + 1]?.parentMessageId;
            if (cut) {
                const p = rows.findIndex((m) => m.id === cut);
                if (p < 0)
                    throw new Error("Missing branch point");
    ...
    createRun(i) {
        this.db
            .prepare("INSERT INTO runs(id,session_id,status,started_at)VALUES(?,?,?,?) ON CONFLICT DO NOTHING")
    ...
    finishRun(id, i) {
        ...
        this.db
            .prepare("UPDATE runs SET status=?,finished_at=? WHERE id=? AND finished_at IS NULL")
    ...
    listUnfinishedRuns() {
        return this.db
            .prepare("SELECT * FROM runs WHERE finished_at IS NULL ORDER BY started_at")
            .all().map((r) => this.getRun(text(r, "id")));
    }
    getRecoveryState(id) {
    ...
    recordFinancialEvidence(i) {
        const p = canonical(i.payload), sha256 = createHash("sha256").update(p).digest("hex");
        this.db
            .prepare("INSERT INTO financial_evidence VALUES(?,?,?,?) ON CONFLICT DO NOTHING")
    ...
    recordToolCall(i) {
        ...
        this.db
            .prepare("INSERT INTO tool_calls VALUES(?,?,?,?,?,?) ON CONFLICT DO NOTHING")
    ...
    recordToolResult(i) {
        ...
        this.db
            .prepare("INSERT INTO tool_results VALUES(?,?,?,?,?) ON CONFLICT DO NOTHING")
    ...
    getToolExchange(id) {
        const r = this.db
            .prepare("SELECT c.*,r.id rid,r.output_json,r.created_at rat,r.evidence_ref FROM tool_calls c LEFT JOIN tool_results r ON r.tool_call_id=c.id WHERE c.id=?")
            .get(id);
        if (!r)
            return undefined;
        const call = {
    ...
    transaction(fn) {
        ...
        try {
            const v = fn(this);
            if (obj(v) && typeof v.then === "function")
    ...
    search(q, o = {}) {
        const before = Math.max(0, o.contextBefore ?? 0), after = Math.max(0, o.contextAfter ?? 0), limit = Math.max(0, o.limit ?? 20);
        let rows;
        try {
            if (this.fts) {
                rows = (o.sessionId
                    ? this.db
                        .prepare("SELECT m.* FROM messages_fts f JOIN messages m ON m.sequence=f.rowid WHERE messages_fts MATCH ? AND m.session_id=? LIMIT ?")
                        .all(q, o.sessionId, limit)
                    : this.db
                        .prepare("SELECT m.* FROM messages_fts f JOIN messages m ON m.sequence=f.rowid WHERE messages_fts MATCH ? LIMIT ?")
                        .all(q, limit));
            }
            else {
                ...
                rows = (o.sessionId
                    ? this.db
                        .prepare("SELECT m.* FROM messages m WHERE m.content LIKE ? ESCAPE '\\' AND m.session_id=? ORDER BY m.sequence LIMIT ?")
                        .all(pattern, o.sessionId, limit)
                    : this.db
                        .prepare("SELECT m.* FROM messages m WHERE m.content LIKE ? ESCAPE '\\' ORDER BY m.sequence LIMIT ?")
                        .all(pattern, limit));
            }
        }
        catch {
        ...
        return rows.map((r) => {
            const message = this.mm(r), all = this.getMessages(message.sessionId, { includeAncestors: true }), i = all.findIndex((m) => m.id === message.id);
            return {
                message,
