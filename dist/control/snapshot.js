 ...
 */
import { SOL_DECIMALS, USDC_DECIMALS, USDC_MINT, USDT_MINT, WSOL_MINT, } from "../kernel/money.js";
...
function symbolFor(mint, fallbackDecimals) {
...
function capLedger(bucket, policy, used) {
...
function phaseOf(policy, inflight, enforced) {
    if (policy.killSwitch || !enforced)
        return "halted";
    if (inflight.some((t) => t.state === "sent"))
        return "settling";
    if (inflight.some((t) => t.state === "reserved"))
        return "executing";
    return "idle";
}
/** Holdings read straight off the chain, with no valuation attached. */
async function readHoldings(runtime, policy) {
...
function readStrategies(runtime) {
...
function readSignals(runtime) {
    ...
    const tape = mints
        .flatMap((mint) => feed.tape.trades(mint).map((t, i) => ({
        id: `${mint}:${t.ts}:${i}`,
        ts: t.ts,
        mint,
...
export function snapshotSources(runtime) {
    return {
...
export async function buildSnapshot(runtime) {
    ...
    const activity = (store?.recentJournal(200) ?? []).map(({ seq, event }) => ({
        id: `j${seq}`,
        at: event.at,
        kind: event.type,
