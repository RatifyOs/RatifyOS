...
export function aggregateCandles(input, intervalSeconds) {
    ...
    for (const t of trades) {
        ...
        if (compare(p, c.high) > 0)
            c.high = p;
        if (compare(p, c.low) < 0)
            c.low = p;
        c.close = p;
...
export function marketAnalytics(trades, activeLiquidity) {
    if (activeLiquidity < 0n)
        throw new RangeError("liquidity must be non-negative");
    let baseVolume = 0n, quoteVolume = 0n;
    let low, high;
    const seen = new Set();
    let tradeCount = 0;
    for (const t of trades) {
        ...
        tradeCount++;
        if (!low || compare(t.price, low) < 0)
            low = canonical(t.price);
        if (!high || compare(t.price, high) > 0)
            high = canonical(t.price);
    }
    return {
        tradeCount,
        baseVolume,
