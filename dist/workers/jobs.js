...
import { nextScheduleTime, } from "../autonomy/jobs/index.js";
export class JobHandlerRegistry {
    m = new Map();
    register(type, schema, handler) {
...
export class JobWorker {
    ...
    async runOnce() {
        ...
        const ac = new AbortController(), beat = setInterval(() => {
            if (this.q.isCancellationRequested(l) ||
        ...
        while (!this.stopRequested) {
            if (this.active.size < (this.o.concurrency ?? 1)) {
                const p = this.runOnce()
                    .then((done) => {
                    if (!done)
                        return new Promise((r) => setTimeout(r, this.o.pollMs ?? 100));
                })
                    .then(() => { });
                this.active.add(p);
...
export class DurableScheduler {
    ...
    o;
    db;
    constructor(path, q, o = {}) {
        this.q = q;
        this.o = o;
        this.db = new DatabaseSync(path);
        this.db.exec("CREATE TABLE IF NOT EXISTS schedules(id TEXT PRIMARY KEY,type TEXT NOT NULL,payload TEXT NOT NULL,schedule TEXT NOT NULL,next_fire INTEGER NOT NULL,enabled INTEGER NOT NULL DEFAULT 1)");
    }
    upsert(id, type, payload, schedule) {
    ...
    tick() {
        const now = (this.o.clock ?? Date.now)(), rows = this.db
