...
export async function main(argv = process.argv.slice(2)) {
    if (!argv.includes("--once"))
        throw new Error("worker currently requires --once");
    ...
    queue.register("noop", z.unknown());
    registry.register("noop", z.unknown(), async () => { });
    try {
        const processed = await new JobWorker(queue, registry, {
            workerId: process.env.WORKER_ID ?? `worker-${process.pid}`,
            leaseMs: 30_000,
        }).runOnce();
        console.log(JSON.stringify({ worker: { once: true, processed } }));
    }
    finally {
        queue.close();
    }
