...
export async function main() {
    ...
    if (!token)
        throw Error("TELEGRAM_BOT_TOKEN is required");
    const config = loadConfig(process.env);
    const api = process.env.RAOS_API_URL ?? `http://${config.host}:${config.port}`;
    const runner = new TelegramRunner({
        ...
        dispatch: async (inbound) => {
            const response = await fetch(new URL("v1/chat", api.endsWith("/") ? api : `${api}/`), {
                method: "POST",
                headers: {
            ...
            if (!response.ok)
                throw Error(typeof body.error === "string"
                    ? body.error
                    : (body.error?.message ??
    ...
    if (!(await runner.check()))
        throw Error("Telegram bot token check failed");
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href)
    main().catch((error) => {
        console.error(error instanceof Error ? error.message : "Telegram runner failed");
        process.exitCode = 1;
    });
