...
import { checkDatabases, databaseStatus, migrateDatabases, } from "../storage/maintenance.js";
export async function main(command = process.argv[2] ?? "config") {
    ...
    const operation = command === "db:migrate"
        ? migrateDatabases
        : command === "db:status"
            ? databaseStatus
    ...
    console.log(JSON.stringify({ command, ok: databases.every((x) => x.integrity === "ok"), databases }, null, 2));
}
main().catch((e) => {
