...
export class ModelRouter {
    ...
    jitter;
    now;
    constructor(candidates, transports, health, options = {}) {
        ...
        for (const candidate of candidates) {
            ...
            if (![candidate.inputCostPerMillion, candidate.outputCostPerMillion].every((v) => Number.isFinite(v) && v >= 0))
    ...
    eligible(request) {
        const tools = request.requiredTools ?? request.tools?.map((tool) => tool.name) ?? [];
        const input = request.contextTokens ?? 0, output = request.maxOutputTokens ?? 0;
        return this.candidates
            .filter((candidate) => candidate.contextWindow >= input + output &&
            tools.every((tool) => candidate.supportedTools.includes(tool)) &&
            this.health.isHealthy(candidate.id))
            .sort((a, b) => a.inputCostPerMillion * input +
    ...
    route(request) {
        ...
        if (!selected)
            throw new Error("No healthy model satisfies required tools and context/output requirements");
        return selected;
    }
    async complete(request) {
        ...
        for (const candidate of this.eligible(request)) {
            ...
            if (!transport) {
                lastError = new Error(`Model transport '${candidate.transport}' for '${candidate.id}' is not configured`);
                continue;
            }
            for (let attempt = 0; attempt <= this.maxRetries; attempt++) {
                ...
                try {
                    ...
                    if (classification.rateLimited) {
                        const delay = error instanceof ModelHttpError &&
                            error.retryAfterMs !== undefined
                            ? error.retryAfterMs
                            : 60_000;
                        this.health.recordRateLimit(candidate.id, this.now() + Math.max(0, delay));
                        break;
                    }
                    if (!classification.retryable || attempt === this.maxRetries)
                        break;
                    await this.sleep(250 * 2 ** attempt + Math.floor(this.jitter() * 100));
                }
            }
            if (!classifyModelError(lastError).fallback)
                throw lastError;
        }
        throw (lastError ?? new Error("No healthy model or model transport available"));
    }
}
...
export class OpenAICompatibleTransport {
    ...
    async complete(request) {
        ...
        const response = await this.options.fetch(`${this.options.baseUrl.replace(/\/+$/, "")}/chat/completions`, {
            method: "POST",
            headers,
        ...
        if (!response.ok)
            throw new ModelHttpError(sanitizeError(await response.text()), response.status, parseRetryAfter(response.headers.get("retry-after"), this.options.now?.() ?? Date.now()));
        ...
        const toolCalls = (choice.message.tool_calls ?? []).map((call, index) => {
            ...
            if (seen.has(id))
                throw new SyntaxError(`Duplicate tool call ID: ${id}`);
            seen.add(id);
            return {
                id,
