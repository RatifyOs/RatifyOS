 ...
 */
export function toIpcPath(socketPath) {
    if (!isWindows)
        return socketPath;
...
export function permissionsAreUnsafe(stats) {
    if (!enforcePosixPermissions)
        return false;
    return (stats.mode & 0o077) !== 0;
}
/** Read the full contents of an inherited file descriptor, portably. */
export async function readFd(fd) {
    const stream = fd === 0 ? process.stdin : createReadStream("", { fd, autoClose: true });
    const chunks = [];
