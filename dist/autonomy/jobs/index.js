...
const MAX_TEXT = 1_000_000, val = (r, k) => r[k], n = (r, k) => Number(r[k]), s = (r, k) => String(r[k]);
const validText = (v, name, allowEmpty = false) => {
    ...
    const walk = (x) => {
        ...
        if (x && typeof x === "object") {
            ...
            const y = Array.isArray(x)
                ? x.map(walk)
                : Object.fromEntries(Object.keys(x)
                    .sort()
                    .map((k) => [k, walk(x[k])]));
            seen.delete(x);
            return y;
...
export class JobQueue {
 ...
 CREATE TRIGGER IF NOT EXISTS attempts_no_delete BEFORE DELETE ON job_attempts BEGIN SELECT RAISE(ABORT,'attempts are append-only'); END;`);
    ...
    open() {
        if (this.closed)
            throw new Error("JobQueue is closed");
    }
    now() {
        const x = this.clock();
    ...
    enqueue(type, payload, o = {}) {
        ...
        const json = canonical(parsed.data), now = this.now(), at = o.scheduledAt ?? now, max = o.maxAttempts ?? 3, back = o.backoffMs ?? 1000, jitter = o.jitter ?? 0, id = o.id ?? randomUUID();
        validText(id, "job id");
        ...
        return this.tx(() => {
            this.db
                .prepare("INSERT INTO jobs(id,type,payload_json,status,scheduled_at,max_attempts,idempotency_key,backoff_ms,jitter,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(idempotency_key) DO NOTHING")
                .run(id, type, json, at > now ? "scheduled" : "queued", at, max, o.idempotencyKey ?? null, back, jitter, now, now);
            if (o.idempotencyKey !== undefined) {
                const r = this.db
                    .prepare("SELECT * FROM jobs WHERE idempotency_key=?")
                    .get(o.idempotencyKey);
        ...
        return this.tx(() => {
            for (;;) {
                const expired = this.db
                    .prepare("SELECT * FROM jobs WHERE status='running' AND lease_expires_at<=? ORDER BY scheduled_at,created_at LIMIT 1")
                    .get(now);
                if (!expired)
                    break;
                const id = s(expired, "id"), cancel = Boolean(n(expired, "cancel_requested")), exhausted = n(expired, "attempt_count") >= n(expired, "max_attempts"), status = cancel ? "cancelled" : exhausted ? "dead-letter" : "queued", outcome = cancel ? "cancelled" : "failed";
                const a = this.db
                    .prepare("UPDATE job_attempts SET finished_at=?,outcome=?,error=? WHERE job_id=? AND fencing_token=? AND finished_at IS NULL")
                    .run(now, outcome, cancel ? null : "Lease expired", id, n(expired, "fencing_token"));
                if (a.changes !== 1)
                    throw new Error("Expired attempt missing or already finalized");
                const j = this.db
                    .prepare("UPDATE jobs SET status=?,worker_id=NULL,lease_expires_at=NULL,error=?,updated_at=? WHERE id=? AND status='running' AND fencing_token=?")
                    .run(status, cancel ? null : "Lease expired", now, id, n(expired, "fencing_token"));
                if (j.changes !== 1)
                    throw new Error("Lease recovery fencing conflict");
            }
            const r = this.db
                .prepare("SELECT id FROM jobs WHERE cancel_requested=0 AND status IN('queued','scheduled') AND scheduled_at<=? AND attempt_count<max_attempts ORDER BY scheduled_at,created_at LIMIT 1")
                .get(now);
            if (!r)
                return undefined;
            const id = s(r, "id"), expires = now + leaseMs;
            const x = this.db
                .prepare("UPDATE jobs SET status='running',worker_id=?,fencing_token=fencing_token+1,lease_expires_at=?,attempt_count=attempt_count+1,updated_at=? WHERE id=? AND status IN('queued','scheduled') AND scheduled_at<=? AND attempt_count<max_attempts")
                .run(workerId, expires, now, id, now);
            if (x.changes !== 1)
                throw new Error("Claim fencing conflict");
            const row = this.db
                .prepare("SELECT * FROM jobs WHERE id=?")
                .get(id);
            this.db
                .prepare("INSERT INTO job_attempts(job_id,number,worker_id,fencing_token,started_at) VALUES(?,?,?,?,?)")
                .run(id, n(row, "attempt_count"), workerId, n(row, "fencing_token"), now);
            return {
                job: this.map(row),
                workerId,
    ...
    heartbeat(l, leaseMs) {
        ...
        if (!Number.isSafeInteger(now + leaseMs))
            throw new Error("Invalid lease");
        const x = this.db
            .prepare("UPDATE jobs SET lease_expires_at=?,updated_at=? WHERE id=? AND worker_id=? AND fencing_token=? AND status='running' AND cancel_requested=0 AND lease_expires_at>?")
    ...
    complete(l, o) {
        ...
        return this.tx(() => {
            const r = this.db
                .prepare("SELECT cancel_requested FROM jobs WHERE id=? AND status='running' AND worker_id=? AND fencing_token=? AND lease_expires_at>?")
                .get(l.job.id, l.workerId, l.fencingToken, now);
            if (!r)
                return false;
            const cancelled = Boolean(n(r, "cancel_requested")), status = cancelled ? "cancelled" : "succeeded";
            const x = this.db
                .prepare("UPDATE jobs SET status=?,result_ref=?,worker_id=NULL,lease_expires_at=NULL,updated_at=? WHERE id=? AND status='running' AND worker_id=? AND fencing_token=? AND lease_expires_at>?")
                .run(status, cancelled ? null : (o.resultRef ?? null), now, l.job.id, l.workerId, l.fencingToken, now);
            if (x.changes !== 1)
                return false;
            const a = this.db
                .prepare("UPDATE job_attempts SET finished_at=?,outcome=?,result_ref=? WHERE job_id=? AND fencing_token=? AND finished_at IS NULL")
                .run(now, cancelled ? "cancelled" : "succeeded", cancelled ? null : (o.resultRef ?? null), l.job.id, l.fencingToken);
            if (a.changes !== 1)
                throw new Error("Attempt finalization failed");
            return !cancelled;
        });
    }
    fail(l, error) {
        ...
        return this.tx(() => {
            const r = this.db
                .prepare("SELECT * FROM jobs WHERE id=? AND status='running' AND worker_id=? AND fencing_token=? AND lease_expires_at>?")
                .get(l.job.id, l.workerId, l.fencingToken, now);
            if (!r)
                return false;
            let status, at = now;
            if (n(r, "cancel_requested"))
                status = "cancelled";
            else if (n(r, "attempt_count") >= n(r, "max_attempts"))
                status = "dead-letter";
            else {
                status = "scheduled";
                const delay = n(r, "backoff_ms") *
            ...
            const x = this.db
                .prepare("UPDATE jobs SET status=?,scheduled_at=?,error=?,worker_id=NULL,lease_expires_at=NULL,updated_at=? WHERE id=? AND status='running' AND worker_id=? AND fencing_token=? AND lease_expires_at>?")
                .run(status, at, error, now, l.job.id, l.workerId, l.fencingToken, now);
            if (x.changes !== 1)
                return false;
            const a = this.db
                .prepare("UPDATE job_attempts SET finished_at=?,outcome=?,error=? WHERE job_id=? AND fencing_token=? AND finished_at IS NULL")
                .run(now, status === "cancelled" ? "cancelled" : "failed", error, l.job.id, l.fencingToken);
            if (a.changes !== 1)
                throw new Error("Attempt finalization failed");
            return true;
        });
    ...
    cancel(id) {
        ...
        return this.tx(() => {
            const r = this.db
                ...
                .get(id);
            if (!r ||
                ["succeeded", "failed", "cancelled", "dead-letter"].includes(s(r, "status")))
                return false;
            if (s(r, "status") === "running")
                this.db
                    .prepare("UPDATE jobs SET cancel_requested=1,updated_at=? WHERE id=? AND status='running'")
                    .run(now, id);
            else
                this.db
                    .prepare("UPDATE jobs SET status='cancelled',cancel_requested=1,updated_at=? WHERE id=? AND status IN('queued','scheduled')")
                    .run(now, id);
            return true;
        });
    }
    isCancellationRequested(l) {
        this.open();
        const r = this.db
            .prepare("SELECT cancel_requested FROM jobs WHERE id=? AND status='running' AND worker_id=? AND fencing_token=?")
            .get(l.job.id, l.workerId, l.fencingToken);
...
const field = (text, min, max, sunday = false) => {
    const out = new Set(), wildcard = text === "*";
    for (const part of text.split(",")) {
        ...
        for (let x = lo; x <= hi; x += step)
            out.add(sunday && x === 7 ? 0 : x);
    }
    return { values: out, wildcard };
};
export function nextScheduleTime(schedule, after) {
    ...
    for (let i = 0; i < 60 * 24 * 366 * 8; i++, t += 60000) {
        const d = new Date(t), domMatch = fs[2].values.has(d.getUTCDate()), dowMatch = fs[4].values.has(d.getUTCDay()), day = fs[2].wildcard
            ? dowMatch
            : fs[4].wildcard
                ? domMatch
                : domMatch || dowMatch;
        if (fs[0].values.has(d.getUTCMinutes()) &&
