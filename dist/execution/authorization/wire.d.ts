...
export type SignerRequest = SignerStatusRequest | SignerSignRequest | SignerResultRequest;
 ...
 */
export type SignerSignResponse = Awaited<ReturnType<SignerService["signEnvelope"]>> & {
    broadcast?: string;
};
/**
...
export type SignerResponseFor<Q extends SignerRequest> = Q extends {
    method: "status";
} ? SignerStatusResponse : Q extends {
    method: "sign";
} ? SignerSignResponse : SignerResultResponse;
...
export interface IsolatedSigner {
    sign(request: Omit<SignerSignRequest, "method" | "token">): Promise<SignerSignResponse>;
    result?(request: Omit<SignerResultRequest, "method" | "token">): Promise<SignerResultResponse>;
}
