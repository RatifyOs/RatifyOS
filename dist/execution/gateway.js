import { assertSimulationSafe, buildSimulationRequest, } from "./simulation.js";
...
export class ExecutionGateway {
    ...
    async prepare(transaction, context) {
        const request = buildSimulationRequest(transaction, context.policyHash), simulation = await this.dependencies.simulate(request);
    ...
    async execute(input) {
        ...
        if (!wireVerifier || !sign || !broadcast)
            throw Error("signing_disabled");
        const verified = await wireVerifier.verify(input.transaction, input.envelope), id = input.envelope.claims.id;
        let signed;
        try {
            signed = await sign(verified.transaction, input.envelope);
        }
        catch (error) {
            await verified.replayStore.transition?.(id, "claimed", "failed", String(error));
            throw error;
        }
        ...
        if (!attached || attached !== signed.signature) {
            await verified.replayStore.transition?.(id, "claimed", "failed", "signer_signature_mismatch");
            throw Error("signer_signature_mismatch");
        }
        await verified.replayStore.transition?.(id, "claimed", "signed", JSON.stringify({
        ...
        if (signed.broadcast !== undefined) {
            if (signed.broadcast !== signed.signature) {
                await verified.replayStore.transition?.(id, "signed", "reconciliation", signed.broadcast);
                throw Error("broadcast_signature_mismatch");
            }
            await verified.replayStore.transition?.(id, "signed", "broadcast", signed.signature);
            return {
                status: "BROADCAST",
        ...
        try {
            ...
            await verified.replayStore.transition?.(id, "signed", "reconciliation", String(error));
            throw error;
        }
        if (signature !== signed.signature) {
            await verified.replayStore.transition?.(id, "signed", "reconciliation", String(signature));
            throw Error("broadcast_signature_mismatch");
        }
        await verified.replayStore.transition?.(id, "signed", "broadcast", signature);
        return {
            status: "BROADCAST",
            messageHash: input.envelope.claims.messageHash,
            signature,
