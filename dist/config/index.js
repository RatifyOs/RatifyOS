...
const llmProviderNames = Object.keys(LLM_PROVIDERS);
/**
 ...
 */
const blankAsAbsent = (inner) => z.preprocess((v) => (v === "" ? undefined : v), inner);
const boolean = z.enum(["true", "false"]).transform((v) => v === "true");
const url = z
    .string()
    .url()
    .refine((v) => ["http:", "https:"].includes(new URL(v).protocol), "must be an HTTP(S) URL");
const schema = z
    .object({
    NODE_ENV: z
    ...
    LLM_CONTEXT_WINDOW: blankAsAbsent(z.coerce.number().int().positive().max(10_000_000).default(8192)),
    LLM_MAX_OUTPUT_TOKENS: blankAsAbsent(z.coerce.number().int().positive().max(1_000_000).default(1024)),
    LLM_INPUT_COST_PER_MILLION: blankAsAbsent(z.coerce.number().nonnegative().default(0)),
    LLM_OUTPUT_COST_PER_MILLION: blankAsAbsent(z.coerce.number().nonnegative().default(0)),
 ...
 */
const walletOrAsset = (v, n) => {
    if (!isPublicKey(v))
        throw Error(`${n} is not a base58 public key`);
    return v;
};
const privateFile = (p, n) => {
    if (!isAbsolute(p))
        throw Error(`${n} must be absolute`);
    const s = lstatSync(p);
    if (!s.isFile() || s.isSymbolicLink())
        throw Error(`${n} must be a regular file`);
    if (permissionsAreUnsafe(s))
        throw Error(`${n} permissions are unsafe`);
};
/**
 ...
 */
function loadLlmConfig(e, provider) {
    if (!e.LLM_MODEL)
        throw Error("LLM_MODEL is required");
    const profile = LLM_PROVIDERS[provider];
    ...
    if (!profile.local) {
        ...
        if (!e.LLM_API_KEY)
            throw Error(`LLM_API_KEY is required for ${provider}`);
    }
...
export function loadConfig(env = process.env, _cwd = process.cwd(), requirements = {}) {
    const e = schema.parse(env);
    const tradingConfigured = e.EXECUTION_MODE === "live" ||
        !!(e.TRADING_ACCOUNT ||
            e.TRADING_MAX_AMOUNT_IN ||
            e.TRADING_ALLOWED_TOKENS);
    const data = e.DATA_DIR ??
        join(e.XDG_STATE_HOME ?? join(homedir(), ".local", "state"), "robinhood-agent-os");
    if (!isAbsolute(data))
        throw Error("DATA_DIR must be an absolute path");
    if (e.NETWORK === "mainnet" &&
        !(e.MAINNET_ENABLED &&
            e.MAINNET_ACKNOWLEDGE_RISK === "I_ACKNOWLEDGE_MAINNET_RISK"))
        throw Error("Mainnet requires double opt-in: MAINNET_ENABLED and MAINNET_ACKNOWLEDGE_RISK");
    ...
    if (tradingConfigured) {
        ...
        if (!e.RPC_URL)
            throw Error("RPC_URL is required");
        trading = {
            ...
            allowedTokens: (e.TRADING_ALLOWED_TOKENS?.split(",").filter(Boolean) ?? []).map((x) => walletOrAsset(x, "TRADING_ALLOWED_TOKENS")),
            ...
            ...(e.APPROVAL_OPERATOR_IDS
                ? {
                    approvalOperators: e.APPROVAL_OPERATOR_IDS.split(",").map((id, i) => ({
                        id,
    ...
    const llmConfigured = !!(e.LLM_PROVIDER ||
        e.LLM_BASE_URL ||
        e.LLM_MODEL ||
        e.LLM_API_KEY);
    ...
    if (requirements.requireRpc && !e.RPC_URL)
        throw Error("RPC_URL is required");
    const marketProviderUrls = (e.MARKET_PROVIDER_URLS?.split(",").filter(Boolean) ?? []).map((v) => url.parse(v));
    return {
        mode: e.NODE_ENV,
        network: e.NETWORK,
...
export function sanitizedConfig(c) {
    return {
        mode: c.mode,
        network: c.network,
