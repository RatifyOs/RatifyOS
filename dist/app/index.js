import { CLUSTER_FOR_NETWORK } from "../config/index.js";
...
import { perpsPositionReader, registerBuiltInTools, } from "../tools/index.js";
import { createModelProvider, RegistryDispatcher } from "./adapters.js";
import { CLUSTER_GENESIS_HASHES, RpcSimulator, } from "../execution/rpc-simulator.js";
import { Connection, PublicKey, TransactionInstruction } from "@solana/web3.js";
import { JupiterClient, SelfRpcBroadcaster, SolanaRpc, } from "../chains/solana/index.js";
...
import { buildSimulationRequest, createSimulationEvidence, } from "../execution/simulation.js";
import { ApprovalEngine, canonicalSerialize, } from "../execution/approvals/index.js";
...
import { buildUnsignedTx, MeteoraDataApi, MeteoraDlmmVenue, PumpFunClient, RealDlmmSdk, RpcChainReader, } from "../pools/index.js";
import { DRIFT_SETTLEMENT_MINT } from "../perps/drift/convert.js";
import { SignalsFeed } from "../data/index.js";
import { gatewayExecutor, StrategyRunner, StrategyStore, strategyMint, } from "../strategy/index.js";
/**
 ...
 */
const clusterAllowlistId = (cluster) => BigInt(`0x${createHash("sha256").update(`solana:cluster:${cluster}`).digest("hex").slice(0, 16)}`);
 ...
 */
const NATIVE_FEE_RESERVE_LAMPORTS = 3000000n;
/**
...
function revokeInstruction(tokenAccount, owner) {
 ...
 */
function allowedPrograms(path) {
    if (!path)
        return new Set();
    try {
        const parsed = JSON.parse(readFileSync(path, "utf8"));
        return new Set((parsed.programs ?? [])
            .map((p) => p?.programId)
            .filter((p) => isPublicKey(p)));
    }
    catch {
        return new Set();
    }
}
export function createTradingComposition(config, fetchFn) {
    if (!config.trading || !config.rpc)
        return undefined;
    const t = config.trading, cluster = CLUSTER_FOR_NETWORK[config.network], store = new ExecutionStore(config.paths.executions);
    const connection = new Connection(config.rpc.url, {
        commitment: "confirmed",
        ...(fetchFn ? { fetch: fetchFn } : {}),
    });
    const solana = new SolanaRpc(connection), jupiter = new JupiterClient(), broadcaster = new SelfRpcBroadcaster(solana);
    const rpcCall = async (method, params) => {
        ...
        if (payload.error)
            throw Error(payload.error.message ?? "rpc error");
     ...
     */
    const pin = async (built) => {
        const request = buildSimulationRequest(built, policyHash), result = await simulator.simulate(request);
        if (!result.success)
    ...
    const rpc = {
        balance: async (owner, mint) => mint ? solana.readBalance(owner, mint) : solana.getSolLamports(owner),
        quote: async (x) => {
            ...
            const slippageBps = Math.min(Number(x.slippageBps ?? t.maxSlippageBps), t.maxSlippageBps);
            ...
            return {
                ...
                slot: quoted.contextSlot === undefined ? slot : BigInt(quoted.contextSlot),
                lastValidBlockHeight: built.lastValidBlockHeight,
        ...
        broadcast: async (wire) => (await broadcaster.broadcast({ wireBase64: wire, signature: signatureOf(wire) }, undefined)).signature,
    ...
    const revokeReservations = new Set();
    let approvals, authorization, reservations, riskEvaluator, risk;
    if (t.liveEnabled) {
        if (!signer ||
        ...
        const operatorKeys = new Map(t.approvalOperators.map((o) => [
            o.id,
        ...
        const riskHashes = new Set(), simulationHashes = new Set();
        const perMint = Object.fromEntries(t.allowedTokens.map((mint) => [mint, t.maxAmountIn]));
        ...
        const canonicalRisk = (value) => canonicalSerialize(JSON.parse(JSON.stringify(value, (_k, v) => typeof v === "bigint" ? { $bigint: v.toString() } : v)));
        const hashRisk = (input, decision) => {
        ...
        const programsAllowed = (r) => r.programIds.length > 0 && r.programIds.every((p) => programs.has(p));
        risk = {
            assess: async (raw) => {
                try {
                    const wrapped = raw;
                    if (wrapped?.quote?.side === "revoke" &&
                        wrapped?.request &&
                        wrapped?.evidence) {
                        ...
                        // issuer will accept.
                        const q = wrapped.quote, r = wrapped.request, reasons = [];
                        const spend = r.instructions.filter((i) => i.programId !== COMPUTE_BUDGET_PROGRAM_ID);
                        if (r.cluster !== cluster)
                            reasons.push("cluster_not_allowed");
                        if (r.feePayer !== t.account)
                            reasons.push("account_not_allowed");
                        if (!programsAllowed(r))
                        ...
                        if ((wrapped.evidence.assetDeltas ?? []).some((d) => d.asset !== "native"))
                    ...
                    if (wrapped?.quote && wrapped?.request && wrapped?.evidence) {
                        const q = wrapped.quote, r = wrapped.request, e = wrapped.evidence, now = BigInt(Math.floor(Date.now() / 1000));
                        const [currentSlot, blockHeight, mintBalance, nativeBalance] = await Promise.all([
                        ...
                        const verdict = riskEvaluator.evaluate(input, reservations.entries(input.now));
                    ...
                    const input = raw;
                    return hashRisk(input, riskEvaluator.evaluate(input, reservations.entries(input.now)));
                }
                catch {
                    return hashRisk(raw, {
                        allowed: false,
        ...
        const authorizationKey = readFileSync(t.authorizationKeyPath, "utf8").trim();
        if (!authorizationKey)
            throw Error("authorization key is empty");
        authorization = new AuthorizationIssuer({
            signerKeyId: t.authorizationKeyId,
            signer: {
                sign: async (data) => createHmac("sha256", authorizationKey).update(data).digest("hex"),
            },
            checks: {
                quote: async (hash) => !!store.findQuoteByHash(hash),
                policy: async (hash) => hash === policyHash,
                risk: async (hash) => riskHashes.has(hash),
                reservation: async (id) => reservations.has(id) || revokeReservations.has(id),
    ...
    const reservationAdapter = reservations
        ? {
            reserve: async (x) => {
                ...
                if (!q)
                    throw Error("reservation_quote_missing");
                if (q.side === "revoke") {
                ...
                if (!reservations.reserveWithin({
                    ...
                    // valuation evidence.
                    perAsset: riskEvaluator.limits.maxReservedPerToken[q.inputMint] ?? 0n,
                }))
                    throw Error("reservation_failed");
                return id;
            },
            valid: async (id) => revokeReservations.has(id) || reservations.has(id),
            commit: async (id) => revokeReservations.delete(id) || reservations.commit(id),
            release: async (id) => revokeReservations.delete(id) || reservations.release(id),
        }
        : undefined;
        ...
        async verify() {
            ...
            if (signer) {
                ...
                if (status.cluster !== cluster)
                    throw Error("signer_cluster_mismatch");
                if (status.policyHash.toLowerCase() !== expectedPolicyHash.toLowerCase())
 ...
 */
function kernelPolicyFor(config) {
    const configuredMints = config.trading?.allowedTokens ?? [];
    const mintAllowlist = configuredMints.length > 0 && configuredMints.every(isPublicKey)
        ? [...configuredMints]
        : null;
    return {
        ...defaultPolicy(),
...
function createVenueComposition(config, wallet, policyController) {
    if (!config.rpc || !isPublicKey(wallet.pubkey))
        return undefined;
    const cluster = CLUSTER_FOR_NETWORK[config.network];
    const connection = new Connection(config.rpc.url, "confirmed");
    const solana = new SolanaRpc(connection), jupiter = new JupiterClient(), chain = new RpcChainReader(connection), kernelStore = new KernelStore(join(config.dataDir, "kernel.sqlite"));
    ...
    const strategies = new StrategyStore(join(config.dataDir, "strategies.sqlite"));
    const strategyRunner = new StrategyRunner(strategies, gatewayExecutor({
        gateway,
        jupiter,
        solana,
    ...
    return {
        ...
        mounts: {
            ...
            pools: {
                ...
                watch: (mint) => signals.watch(mint),
            },
        },
    };
}
export function createApplication(config, overrides = {}) {
    let state = "created", rpcHealthy = !config.rpc, tradingHealthy = true;
    let sessions;
    let reconciliationTimer, reconciliationRunning = false;
    ...
    if (tc)
        registerTradingTools(registry, tc.trading);
    const capabilities = [
        ...
        async health() {
            ...
            return {
                ...
                dependencies: {
                    ...
                    model: {
                        status: overrides.modelProvider || config.llm
                            ? "available"
                            : "unconfigured",
